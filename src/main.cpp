...
string temp_score = scoreManager.getScore();  
string temp_highscore = scoreManager.getHighScore();
...
int main(int args, char* argv[]) {
    ...
    temp_highscore = "BEST: " + to_string(scoreManager.getHighScoreInt()); 
    const char* highscore = temp_highscore.c_str();
    //sprintf(highscore, "BEST: %d", highscoreInt);
    ...
    while (!quit) {
        ...
        if (titleScreen) {
            if (splashTimer > 120) {
                ...
                Draw_Font(renderer, score, screenWidth/2 - 37, screenHeight/2 + 10, 74, 32, 32, {0, 0, 0});
